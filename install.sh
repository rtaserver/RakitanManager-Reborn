...
install_rakitanmanager() {
    ...
    # Step 3: Download
    step_header 3 "Downloading RakitanManager-Reborn"
    (
        create_directories
    ...
    (
        ...
        if [ -f "$CONFIG_FILE" ]; then
            ...
            log "Removed existing configuration file" "INFO"
        fi
        ...
        if [ -d "/usr/share/rakitanmanager" ]; then
            ...
            log "Removed existing core files" "INFO"
        fi
        ...
        if [ -f "/etc/init.d/rakitanmanager" ]; then
            ...
            log "Removed existing init.d script" "INFO"
        fi
        ...
        if [ -d "/www/rakitanmanager" ]; then
            ...
            log "Removed existing web interface files" "INFO"
        fi
...
create_minimal_installation() {
    ...
    if [ -f /usr/lib/lua/luci/controller/rakitanmanager.lua ] || [ -f /usr/lib/lua/luci/view/rakitanmanager.htm ]; then
        rm -f /usr/lib/lua/luci/controller/rakitanmanager.lua /usr/lib/lua/luci/view/rakitanmanager.htm 2>/dev/null
    fi
...
show_menu() {
    ...
    echo -e "║${BOLD}                     Installer Version 3.1                 ${CYAN}║"
    echo -e "╚═══════════════════════════════════════════════════════════╝${NC}"
    echo -e "\n"
